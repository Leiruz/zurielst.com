...
/**
 * Self-contained so its built source can run synchronously in the document head.
 */
export function stampInitialIntro(
  dependencies: IntroStampDependencies = {},
): void {
  ...
  try {
    ...
    const matchMedia =
      dependencies.matchMedia ?? window.matchMedia.bind(window);
    const schedule = dependencies.schedule ?? window.setTimeout.bind(window);
    const seen = storage.getItem('zst-hello-seen') === '1';
    ...
    const storageIsWritable = storage.getItem(probeKey) === '1';
...
const INTRO_STAMP_SOURCE = `(${stampInitialIntro.toString()})();`;
const NO_SCRIPT_STYLE = '#intro-cover{display:none!important}';

export function IntroFirstPaintHead() {
  return (
    <Fragment>
      <script
        id="intro-state-stamp"
        dangerouslySetInnerHTML={{ __html: INTRO_STAMP_SOURCE }}
      />
      <noscript>
