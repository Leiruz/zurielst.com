// Vendored from ncdai registry item "apple-hello-effect" (chanhdai.com/r, MIT).
...
type HelloAnimationStyle = CSSProperties & {
  "--hello-delay": string
  "--hello-duration": string
}
...
export function AppleHelloEffectEnglish({
  ...
  const animationStyle = (duration: number, delay: number): HelloAnimationStyle => ({
    "--hello-delay": `${calc(delay)}s`,
    "--hello-duration": `${calc(duration)}s`,
  })
  const handleAnimationEnd = (event: AnimationEvent<SVGPathElement>) => {
    if (event.animationName === "apple-hello-draw") onAnimationComplete?.()
  }

  return (
    <svg
      className={className ? `h-20 ${className}` : "h-20"}
