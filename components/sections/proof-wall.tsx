...
function ProofImage({ src, alt }: { src: string | undefined; alt: string }) {
  ...
  return <Image src={src!} alt={alt} width={720} height={405} className="aspect-video w-full object-cover" />;
}
  ...
  return (
    <ProofTile>
      ...
      {mediaAvailable && item.type === 'image' && (
        <Image src={item.media} alt={`${item.title} artifact`} width={720} height={405} className="aspect-video w-full object-cover" />
      )}
      <div className="flex flex-1 flex-col p-5">
