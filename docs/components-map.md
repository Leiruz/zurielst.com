...
- consent-manager: mounts a site-owned Cloudflare Web Analytics loader that injects its deferred beacon only after c15t measurement consent. A genuine measurement revocation reloads after a granted-to-denied transition when that page load injected or recovered the owned beacon; tag removal is best-effort cleanup.
