...
- consent-manager: uses c15t's headless provider and state with a site-owned automatic banner, and loads the split c15t dialog only after Customize opens it. The lightweight provider, hydration, and analytics runtime mounts after either the two-frame readiness signal or intro completion. A cookie-first raw validation barrier runs before provider construction, rejects incomplete local records while accepting c15t's omitted-false cookie encoding, and deletes invalid storage through c15t's public API. Valid matching records are restored unconditionally through c15t's custom-consent save path. Absent or invalid records reset the live provider to necessary-only defaults, and analytics remains blocked until the live state matches the validated result. The privacy choices listener, banner, and dialog payload remain gated until engagement, readiness, and intro completion have all occurred, which prevents consent UI from overlapping the intro.
