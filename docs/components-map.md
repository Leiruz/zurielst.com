...
- spotlight-logo: removed 2026-08-31 when the navbar adopted the full-name wordmark; the vendored component and every .spotlight-logo CSS rule were deleted rather than kept unmounted.
...
- shimmering-text: composes with each rotating identity role and the intro gate's single Slide to unlock label. It preserves the one-second default pace through progressive CSS and becomes static under reduced motion. The navbar wordmark and identity tagline are plain text.
...
- fluid-gradient-text: remains vendored with unique SVG gradient IDs, dossier typography, an accessible label, and a centered reduced-motion state. Its viewport-gated `next/dynamic` wrapper keeps a static Zuriel fallback until the large, full-column footer bookend approaches view immediately above the colophon.
...
- experience-01: frames the profile-driven Timeline with the staged max-width, container, border, and structural-line layout. A shared dossier shell applies only at the 80rem line-nav breakpoint, keeping the original mobile and tablet inset while clearing the desktop rail. The existing Fig. 7 title and section anchor remain the only Timeline heading, while work-experience receives the seven organizations and eight native collapsed disclosures.
...
- consent-manager: uses c15t's headless provider and state with a site-owned automatic banner, and loads the split c15t dialog only after Customize opens it. A small site-owned runtime loads dynamically after either the two-frame readiness signal or intro completion, validates cookie-first persisted consent without importing the c15t store, and starts analytics for a validated measurement grant. It rejects incomplete local records, accepts c15t's omitted-false cookie encoding, and deletes invalid local and cookie storage best-effort. The c15t provider, privacy choices listener, banner, and dialog remain gated until engagement, readiness, and intro completion have all occurred. The validated result is then restored unconditionally through c15t's custom-consent save path, so an all-denied return visit keeps the banner suppressed.
- consent-manager: mounts a site-owned Cloudflare Web Analytics loader from the readiness runtime and keeps it synchronized with c15t after the provider loads. A genuine measurement revocation marks the current document revoked, performs best-effort owned-tag cleanup, saves the current denied state through c15t's public storage API, and reloads only after c15t's cookie-first canonical readback confirms denial. Failed or stale readback stays fail-closed without reloading into granted consent.
...
- theme-switcher: keeps a CSS outer reveal and an action-labelled 36px toggle with a 24px keyed IconSwap for the active-theme icon, removes the explicit system option while preserving system defaults in the provider, and waits for a resolved active theme before rendering the control. Its loading placeholders reserve the same 36px footprint.
